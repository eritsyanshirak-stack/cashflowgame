  ...
  return <section className="v14-sales-desk">
    ...
    <div className="v14-listings">{player.assets.map((asset) => {
      ...
      return <article key={asset.id}>
        ...
        <div className="v14-share-actions"><span>Продать часть бизнеса:</span>{([10, 25, 50] as const).map((share) => <button key={share} disabled={game.phase !== 'ready' || asset.ownership * 100 <= share || Boolean(pledgedLoanForAsset(player, asset.id))} onClick={() => dispatch({ type: 'SELL_ASSET_SHARE', assetId: asset.id, percent: share })}>{share}%</button>)}</div>
