...
path = Path('src/game/engine/engine.ts')
source = path.read_text()

def once(old: str, new: str) -> None:
    global source
    if old not in source:
        raise RuntimeError(f'Missing anchor: {old[:120]}')
...
def between(start: str, end: str, replacement: str) -> None:
    ...
    if left < 0:
        raise RuntimeError(f'Missing start: {start}')
    right = source.find(end, left)
    if right < 0:
        raise RuntimeError(f'Missing end: {end}')
...
once("  const player = state.players[0]\n\n  if (command.type === 'RESOLVE_ASSET_ISSUE')", """  const player = state.players[0]
...
once("  const decision = state.pendingDecision\n\n  if (command.type === 'NEGOTIATE_BUSINESS')", """  const decision = state.pendingDecision
...
once("  if (command.type === 'BUY_BUSINESS') {", """  if (command.type === 'ACCEPT_PARTNERSHIP') {
...
print('materialized v1.4 engine')
