...
def once(old: str, new: str) -> None:
    ...
    if old not in source:
        raise RuntimeError(f'Missing App anchor: {old[:120]}')
...
def between(start: str, end: str, replacement: str) -> None:
    ...
    if left < 0:
        raise RuntimeError(f'Missing App start: {start}')
    right = source.find(end, left)
    if right < 0:
        raise RuntimeError(f'Missing App end: {end}')
